...
class Code128Test < Barby::TestCase
  ...
  it 'should not allow empty data' do
    -> { Code128B.new('') }.must_raise(ArgumentError)
  end
  ...
  describe 'multiple encodings' do
    ...
    it "should raise an exception when extra string doesn't start with the special code character" do
      -> { @code.extra = '123' }.must_raise ArgumentError
    end
  ...
  describe 'Function characters' do
    ...
    it 'should not allow FNC > 1 for Code C' do
      -> { Code128C.new("12\302") }.must_raise ArgumentError
      -> { Code128C.new("\30312") }.must_raise ArgumentError
      -> { Code128C.new("12\304") }.must_raise ArgumentError
    end
  ...
  describe 'Code128 with type' do
    ...
    it 'should raise an exception when given a non-existent type' do
      -> { Code128.new('abc', 'F') }.must_raise(ArgumentError)
    end

    it 'should not fail on frozen type' do
      Code128.new('123456', 'C') # not failing
      Code128.new('123456', 'c') # not failing even when upcasing
    end
  ...
  describe 'Code128 automatic charset' do
    ...
    it 'should know how to extract CODEC segments properly from a data string' do
      ...
      Code128.send(:extract_codec, '12345abc6').must_equal %w[1234 5abc6]
      Code128.send(:extract_codec, 'abcdef').must_equal ['abcdef']
      Code128.send(:extract_codec, '123abcdef45678').must_equal %w[123abcdef4 5678]
      Code128.send(:extract_codec, 'abcd12345').must_equal %w[abcd1 2345]
      Code128.send(:extract_codec, 'abcd12345efg').must_equal %w[abcd1 2345 efg]
      Code128.send(:extract_codec, '12345').must_equal %w[1234 5]
      Code128.send(:extract_codec, '12345abc').must_equal %w[1234 5abc]
      Code128.send(:extract_codec, 'abcdef1234567').must_equal %w[abcdef1 234567]
    end
