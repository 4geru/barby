...
class EAN13Test < Barby::TestCase
  describe 'validations' do
    ...
    it 'should raise an exception when data is invalid' do
      -> { EAN13.new('123') }.must_raise(ArgumentError)
    end
  end
  ...
  describe 'data' do
    ...
    it 'should have the expected numbers' do
      @code.numbers.must_equal @data.split(//).map(&:to_i)
    end
    ...
    it 'should have the expected numbers_with_checksum' do
      @code.numbers_with_checksum.must_equal @data.split(//).map(&:to_i) + [5]
    end
  ...
  describe 'encoding' do
    ...
    it 'should have the expected left_parity_map' do
      @code.left_parity_map.must_equal %i[odd even odd even odd even]
    end
