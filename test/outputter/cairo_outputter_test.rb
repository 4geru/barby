...
class CairoOutputterTest < Barby::TestCase
  ...
  def eps_available?
    ps_available? && Cairo::PSSurface.method_defined?(:eps=)
  end
  ...
  it 'should return PS document by the to_ps method' do
    @barcode.to_ps.must_match(/\A%!PS-Adobe-[\d.]/) if ps_available?
  end
  ...
  it 'should return SVG document by the to_svg method' do
    @barcode.to_svg.must_match(%r{</svg>\s*\Z}m) if svg_available?
  end
